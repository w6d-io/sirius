...
use kafka::{KafkaMessage, KafkaProducer, producer::{FutureProducer, future_producer::DefaultFutureContext}};
...
fn get_producer(config: &Arc<SiriusConfig>, producer: &str, request_id: &str)-> KafkaProducer<FutureProducer, DefaultFutureContext>{
...
pub async fn sync_user(
    ...
    match sync(config, identitys, &request_id, mode).await{
        Ok(_) => {
            ...
            if let Err(e) = kafka_notif.produce(message, Some(Duration::from_secs(30))).await{
                let message = KafkaMessage{
                    payload: e.to_string(),
                    key: None,
                    headers: None,
                };
                if let Err(e) = kafka_error.produce(message, Some(Duration::from_secs(30))).await{
                    error!("{request_id}: {e}");
                    panic!();
                }
            }
        ...
        Err(e) => {
            ...
            let res = kafka_error.produce(message, Some(Duration::from_secs(30))).await;            let message = KafkaMessage{
